...
sub do_spans($@)
{
	...
	return join "<br />",map
	{
		...
		# make URLs clickable and hide them:
		$line=~s{$url_re}{push @hidden,"<a href=\"$1\" rel=\"nofollow\">$1\</a>"; "<!--$#hidden-->$2"}sge;

		# spoiler:
		$line=~s{ (?<![0-9a-zA-Z%\x80-\x9f\xe0-\xfc]) (%%) (?![<>\s%]) ([^<>]+?) (?<![<>\s%\x80-\x9f\xe0-\xfc]) \1 (?![0-9a-zA-Z%]) }{<span class="spoiler">$2</span>}gx;
...
sub cookie_encode($;$)
{
	...
	else # do the hard work ourselves
	{
		if($charset=~/\butf-?8$/i)
		{
			$str=~s{([\xe0-\xef][\x80-\xBF][\x80-\xBF]|[\xc0-\xdf][\x80-\xBF]|&#([0-9]+);|&#[xX]([0-9a-fA-F]+);|[^0-9a-zA-Z])}{
			# convert UTF-8 to URL encoding - only handles up to U-FFFF
				my $c;
				if($2) { $c=$2 }
				elsif($3) { $c=hex $3 }
		...
		elsif($charset=~/\b(?:shift.*jis|sjis)$/i) # old perl, using shift_jis
		{
			...
			$str=~s{([\x80-\x9f\xe0-\xfc].|&#([0-9]+);|&#[xX]([0-9a-fA-F]+);|[^0-9a-zA-Z])}{
			# convert Shift_JIS to URL encoding
...
if (0) {# v DISABLED v	8x------	--------	--------	--------
	...
	#	$thumb->trueColorToPalette(); # <- sometimes discards more valuable colors
		...
		print THUMBNAIL $out;
		close THUMBNAIL;
		return 1 if -e $thumbnail; # unless($!);
	}
