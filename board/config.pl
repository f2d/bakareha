...
# System config
use constant ADMIN_PASS => 'CHANGE_ME';		# Admin password. CHANGE THIS to something you will remember.
#use constant NUKE_PASS => 'CHANGE_ME';		# Optional password to discard all board content. Leave out to disable.
use constant SECRET => 'CHANGE_ME';		# Cryptographic secret. CHANGE THIS to something totally random, and long.
#use constant CAPPED_TRIPS => ('!!example1'=>' capcode','!!example2'=>' <span class="cap">Cap-tan</span>');
				# Usage: posterName!!tripCodePassword
				# Admin tripcode hash, for startng threads when locked down, and similar.
				# Format is '!trip'=>'capcode', where 'capcode' is what is shown instead of the trip.
				# This can contain HTML, but keep it valid XHTML!

# Page look
use constant TITLE => 'Feedback and help';	# Name of this image board
...
#use constant ALLOW_ADMIN_EDIT => 0;		# Allow editing of include files and spam.txt from admin.pl.
				# Warning! This is a security risk, since include templates can run code!
				# Only enable if you completely trust your moderators!
...
use constant MAX_KB => 9999;			# Maximum upload size in KB
...
#use constant CONVERT_COMMAND => 'w:\\usr\\local\\im\\convert.exe';
				# location of the ImageMagick convert command (usually just 'convert', but sometime a full path is needed)
...
use constant DATE_STYLE => 'ymdhms';		# Date style ('2ch', 'futaba', 'localtime, 'http')
#use constant DISPLAY_ID => '';			# How to display user IDs
				# 0 or '': don't display,
...
#use constant CSS_FILE => 'kareha.css';		# Location of the css file
...
#use constant ADMIN_EDITABLE_FILES => (SPAM_FILES);	# A Perl list of all files that can be edited from the admin script.
				# Hints:
				# * If you don't trust your moderators, don't let them edit templates! Templates can execute code on your server!
				# * If you still want to allow editing of templates, use (SPAM_FILES,glob("include/*")) as a convenient shorthand.
