...
sub simple_format($$)
{
	...
	return join "<br />",map
	{
		...
		$line=~s!&gt;&gt;($replyrange_re)!\<a href="$thread#$1" rel="nofollow"\>&gt;&gt;$1\</a\>!gm;
...
sub wakabamark_format($$)
{
	...
	{
		my $line=shift;
		$line=~s!&gtgt;($replyrange_re)!\<a href="$thread#$1" rel="nofollow"\>&gt;&gt;$1\</a\>!gm;
		return $line;
	};
...
sub html_format($$)
{
	...
	$text=~s!&gt;&gt;($replyrange_re)!\<a href="$thread#$1" rel="nofollow"\>&gt;&gt;$1\</a\>!gm;
	$text=~s!(?:\r\n|\n|\r)!<br />!sg;
...
sub make_thread($$$)
{
	...
	for my $directories (RES_DIR, IMG_DIR, THUMB_DIR) {
		if (!-d $directories) {
			mkdir $directories or make_error("Failed to create path: $directories");
		}
	}
...
sub process_file($$)
{
	...
	my $thumbnail=THUMB_DIR.$filebase."s.png";
